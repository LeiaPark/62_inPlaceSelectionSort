...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
    	...
    	for (int i = 0; i < unordered.size(); i++) {
    		...
    		int champNum = unordered.get(champInd);
    		unordered.set(champInd, unordered.get(i));
    		unordered.set(i, champNum);
     ...
     */
     private int champIndex(int start) {
        int champNum = Integer.MAX_VALUE;
        int champInd = -1;
        for ( int i = start; i < list_iAS.size(); i++) {
        	if (list_iAS.get(i) < champNum) {
        		champNum = list_iAS.get(i);
                champInd = i;
        	}
        }
        return champInd;
